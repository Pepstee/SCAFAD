...
============================================
Unit tests for the isolation_forest detector module.
...
from layer0.detectors.isolation_forest import detect
...
class TestIsolationForestDetector(unittest.TestCase):
    ...
    def test_registered_in_registry(self):
        """isolation_forest must be registered in the DetectorRegistry."""
        self.assertIn('isolation_forest', REGISTRY)
    ...
    def test_returns_detection_result_with_no_history(self):
        ...
        isolation_forest delegates to statistical_outlier when sklearn is
        unavailable or insufficient data exists, so the returned algorithm_name
        may be either 'isolation_forest' or 'statistical_outlier'.
        """
        result = detect(self.telemetry, deque(), self.ml_models, self.config)
        self.assertIsInstance(result, DetectionResult)
        # Fallback to statistical_outlier is expected with no history
        self.assertIn(
